    ...
    for item in tqdm(user_data):
        if item.tier not in ['Pro', 'Plus']:
            try:
                ...
                data = pb.collection("darkPoolFlow").get_full_list(query_params = {"filter": f"user = '{item.id}'"})
                for screener in data:
                    pb.collection('darkPoolFlow').delete(screener.id)
                ...
                #Portfolio-Tracker
                portfolio_data = pb.collection("portfolio").get_full_list(query_params={"filter": f"user = '{item.id}'"})

                if len(portfolio_data) > 1:

                    # Keep the first one
                    keep = portfolio_data[0]

                    # Limit tickers to max 5
                    tickers = keep.ticker  # assuming list
                    if len(tickers) > 5:
                        new_tickers = tickers[:5]
                        pb.collection("portfolio").update(keep.id, {"ticker": new_tickers})

                    # Delete all others
                    for portfolio in portfolio_data[1:]:
                        pb.collection("portfolio").delete(portfolio.id)

                elif len(portfolio_data) == 1:
                    portfolio = portfolio_data[0]
                    tickers = portfolio.ticker  # assuming list

                    if len(tickers) > 5:
                        new_tickers = tickers[:5]
                        # Update the portfolio with the first 5 tickers only
                        pb.collection("portfolio").update(portfolio.id, {"ticker": new_tickers})
