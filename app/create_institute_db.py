...
def normalize_name(name):
    ...
    try:
        if name is not None:
            formatted_name = re.sub(r'\s+', ' ', re.sub(r'[^\w\s]', '', name.lower())).strip()
        else:
            formatted_name = None
                    ...
                    if isinstance(parsed_data, list):
                        for item in parsed_data:
                            if item['symbol'] is None:
                                ...
                                if normalized_security_name in stock_dict and normalized_security_name is not None:
                                    item['symbol'] = stock_dict[normalized_security_name]
                                elif normalized_security_name in etf_dict and normalized_security_name is not None:
