...
def run_options_jobs():
    ...
    if week <= 5:
        ...
        run_command(["python3", "cron_options_list.py"])
...
def run_cron_var():
    ...
    if week <= 4:
        ...
        run_command(["python3", "cron_spy_average.py"])

def run_cron_sector():
    week = datetime.today().weekday()
    if week <= 4:
        run_command(["python3", "cron_sector.py"])
        run_command(["python3", "cron_etf_profile.py"])
...
def run_financial_statements():
    ...
    if week <= 5:
        ...
        run_command(["python3", "cron_valuation.py"])
...
schedule.every(5).minutes.do(run_threaded, run_market_moods).tag('market_moods_job')
...
schedule.every(30).seconds.do(run_threaded, run_market_flow).tag('market_flow_job')
