...
N_weeks_ago = datetime.now(utc_timezone) - timedelta(weeks=50)
    ...
    if os.path.exists(file_path):
        try:
            ...
            for item in existing_data:
                try:
                    # Parse the stored New York timestamp and compare in UTC
                    date_obj = datetime.strptime(item['date'], "%Y-%m-%d %H:%M:%S")
                    if date_obj.tzinfo is None:
                        date_obj = ny_timezone.localize(date_obj)
                    else:
                        date_obj = date_obj.astimezone(ny_timezone)

                    date_obj_utc = date_obj.astimezone(utc_timezone)

                    if date_obj_utc >= N_weeks_ago:
                        # Normalize back to NY wall time before persisting
            ...
            async with session.get(url, params=querystring, headers=headers) as response:
                ...
                for item in res:
                    try:
                        ...
                        # Skip items older than two weeks (compare in UTC to avoid locale issues)
                        if date_obj.astimezone(utc_timezone) < N_weeks_ago:
                            continue
