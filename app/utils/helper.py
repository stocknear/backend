...
def get_exchange_to_usd_factor(currency: str, cache_factor: dict) -> float:
    if not currency:
        return 1.0
    cur = currency.upper()
    # quick short-circuit
    if cur == "USD":
        cache_factor["USD"] = 1.0
        return 1.0

    # cache hit
    if cur in cache_factor:
        return cache_factor[cur]

    try:
        url = "https://financialmodelingprep.com/stable/quote"
        params = {"symbol": f"{cur}USD", "apikey": api_key}
        r = requests.get(url, params=params, timeout=10)
        r.raise_for_status()
        data = r.json()

        # FMP returns a list with one object:
        # [{"symbol":"DKKUSD","name":"DKK/USD","price":0.15523,...}]
        if not isinstance(data, list) or not data:
            raise ValueError(f"Unexpected FMP payload: {data!r}")

        price = data[0].get("price")
        if price is None:
            raise ValueError(f"No 'price' in FMP payload: {data[0]!r}")

        factor = float(price)
        print(f"{cur}→USD: {factor}")
        cache_factor[cur] = factor
        return factor
    except Exception as e:
        print("FX fetch error (FMP):", e)
        return 1.0  # safe default


def calculate_forward_pe(symbol: str, next_year: str, cache_factor: dict, api_key: str) -> float | None:
    """
    Calculates Forward P/E = price / next_year_EPS_in_USD
    Expects local JSONs:
      json/analyst-estimate/{symbol}.json  -> list of estimates dicts (must contain 'date' and 'estimatedEpsAvg')
      json/quote/{symbol}.json             -> {'price': <float>}
      json/financial-statements/income-statement/annual/{symbol}.json -> first item has 'reportedCurrency'
    """
    ...
    try:
        with estimates_path.open("rb") as f:
            estimates = orjson.loads(f.read())
        with quote_path.open("rb") as f:
            price_data = orjson.loads(f.read())
        ...
        if item and price:
            # get FX factor (cached in-memory)
            factor = cache_factor.get(currency.upper())
            if factor is None:
                factor = get_exchange_to_usd_factor(currency, cache_factor, api_key)

            eps_local = item.get("estimatedEpsAvg")
            if eps_local and factor:
                eps_usd = float(eps_local) * float(factor)
                if eps_usd:
                    return round(float(price) / eps_usd, 2)
    except Exception as e:
        print("PE calc error:", e)
        return None
