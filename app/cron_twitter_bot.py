...
def wiim():
    try:
        with open("json/twitter/wiim.json", "r") as f:
    ...
    if res_list:
        ...
        if result != None and result['trackingID'] not in seen_ids:
            ...
            try:
                ...
                print("Dark pool tweet sent successfully!")
            except Exception as e:
                print(f"Error sending dark pool tweet: {e}")
        else:
...
def options_flow():
    ...
    try:
        with open("json/twitter/options_flow.json", "rb") as file:
            seen_list = orjson.loads(file.read())
    ...
    try:
        ...
        print("Options flow tweet sent successfully!")

    except Exception as e:
        print(f"Error sending options tweet: {e}")


def recent_earnings():

    try:
        with open("json/twitter/recent_earnings.json", "r") as file:
            seen_list = orjson.loads(file.read())
            seen_list = [item for item in seen_list if datetime.fromisoformat(item['date']).date() == today]
    except:
        seen_list = []
    ...
    if res_list:
        ...
        try:
            with open("json/twitter/recent_earnings.json","wb") as file:
