...
def compute_upside(price_target, current_price, decimals=2):
    pt = safe_float(price_target)
    price = safe_float(current_price)
    if pt is None or price is None or price <= 0:
        return None
    return round(((pt / price) - 1) * 100, decimals)
...
def process_top_analyst_data(data, current_price):
    try:
        # Keep only top analysts (score >= 4)
        filtered_data = []
        for item in data:
            try:
        ...
        data = filter_latest_analyst_unique_rating(filtered_data)

        # Sort by date descending and keep last 12 months (max 30 entries)
        sorted_recent = []
        for item in data:
            try:
                dt = datetime.strptime(item['date'], "%Y-%m-%d")
                if dt >= one_year_ago:
                    sorted_recent.append((dt, item))
            except Exception:
                continue

        sorted_recent.sort(key=lambda x: x[0], reverse=True)
        recent_data = [record for _, record in sorted_recent[:30]]
        ...
        price_targets = [
            safe_float(item.get('adjusted_pt_current'))
            for item in recent_data
        ]
        price_targets = [pt for pt in price_targets if pt is not None]

        median_price_target = None
        if price_targets:
            price_targets.sort()
            median_price_target = price_targets[len(price_targets) // 2]
            if median_price_target is not None and median_price_target > 0:
                median_price_target = round(median_price_target, 2)
            else:
                median_price_target = None

        upside = compute_upside(median_price_target, current_price)
...
async def get_stock_screener(con):
    ...
    #test mode
    #filtered_data = [item for item in stock_screener_data if item['symbol'] == 'BYND']
    ...
    for item in tqdm(stock_screener_data):
        ...
        try:
            with open(f"json/analyst/summary/all_analyst/{symbol}.json", 'r') as file:
                res = orjson.loads(file.read())
                item['analystRating'] = res.get('consensusRating')
                item['analystCounter'] = res.get('numOfAnalyst')
                price_target_value = safe_float(res.get('medianPriceTarget'))
                item['priceTarget'] = round(price_target_value, 2) if price_target_value is not None else None
                item['upside'] = compute_upside(price_target_value, item.get('price'))
        except:
        ...
        try:
            with open(f"json/analyst/history/{symbol}.json") as file:
                ...
                res_dict = process_top_analyst_data(data, safe_float(item.get('price')))
        ...
        try:
            with open(f"json/financial-statements/key-metrics/annual/{symbol}.json", 'r') as file:
                ...
                item['grahamNumber'] = round(res['grahamNumber'],2) if res['grahamNumber'] != None else None
                item['grahamUpside'] = round((item['grahamNumber'] / item['price'] - 1) * 100, 2) if item['grahamNumber'] != None else None
        ...
        try:
            with open(f"json/dividends/companies/{symbol}.json", 'r') as file:
                ...
                if too_old:
                    ...
                    item['payoutRatio'] = round(res['payoutRatio'], 2) if res['payoutRatio'] != None else None
                    item['dividendGrowth'] = round(res['dividendGrowth'], 2) if res['dividendGrowth'] != None else None
...
async def get_economic_calendar():
    ...
    for item in data:
        try:
            ...
            # Parse the event timestamp as UTC and convert to New York time
