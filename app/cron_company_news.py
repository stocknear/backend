...
async def filter_and_deduplicate(data, excluded_domains=None, deduplicate_key='title'):
    ...
    for item in data:
        ...
        if not image_url or any(domain in url for domain in excluded_domains):
            continue
...
async def process_chunk(session, chunk, rate_limiter, image_validator):
    """
    ...
    for symbol in chunk:
        try:
            ...
            filtered_data = await image_validator.filter(filtered_data)
            if filtered_data:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [process_chunk(session, chunk, rate_limiter, image_validator) for chunk in chunks]
